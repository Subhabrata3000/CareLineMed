import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:fluttertoast/fluttertoast.dart';
import 'package:get/get.dart';
import 'package:http/http.dart' as http;
import 'package:laundry/Api/config.dart';
import '../Api/data_store.dart';
import '../model_doctor/wallet_detail_model.dart';



class WalletDetailController extends GetxController implements GetxService {
  bool isLoading = false;
  WalletDetailModel? walletDetailModel;

  walletDetailApi({context}) async{

    Map body = {
      "id": getData.read("UserLogin")["id"].toString()
    };

    Map<String, String> userHeader = {"Content-type": "application/json", "Accept": "application/json"};
    var response = await http.post(Uri.parse(Config.baseUrlDoctor + Config.walletDetail),body: jsonEncode(body),headers: userHeader);

    var data = jsonDecode(response.body);


    if(response.statusCode == 200){
      if(data["Result"] == true){
        walletDetailModel = walletDetailModelFromJson(response.body);
        if(walletDetailModel!.result == true){
          isLoading = true;
          update();
        }
        else{
          Fluttertoast.showToast(msg: walletDetailModel!.message.toString());
        }
      }
      else{
        Fluttertoast.showToast(msg: "${data["message"]}");
      }
    }
    else{
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please update the content from the backend panel. It appears that the correct data was not uploaded, or there may be issues with the data that was added.")));
    }
  }

}